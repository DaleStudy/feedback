...
describe('anonymousRespondentKey', () => {
  test('같은 입력이면 같은 키, 설문이나 사용자가 다르면 다른 키', async () => {
    const a = await anonymousRespondentKey('secret', 'blog01-final', 1)
    expect(a).toMatch(/^[0-9a-f]{64}$/)
    expect(await anonymousRespondentKey('secret', 'blog01-final', 1)).toBe(a)
    expect(await anonymousRespondentKey('secret', 'blog01-final', 2)).not.toBe(a)
    expect(await anonymousRespondentKey('secret', 'blog02-final', 1)).not.toBe(a)
  })
