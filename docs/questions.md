...
모든 스터디가 같은 `key` 로 묻는 문항. `src/questions/common.ts` 에 있고, seed 에서는 `{ "common": "<key>" }` 로 가져온다. 기수·스터디 간 비교는 `questions.key` 로 한다.

| key | type | 문항 | 비고 |
|---|---|---|---|
| `goal_achieved` | scale | 시작할 때 이 스터디에서 하고 싶었던 것을 얼마나 해냈나요? | |
...
| `highlight_lowlight` | long | 이번 기수에서 잘한 것 하나와 아쉬운 것 하나를 적어주세요. | |
| `do_differently` | long | 같은 스터디를 다시 한다면 무엇을 다르게 하시겠어요? | |
| `community_help` | long | 그렇게 하는 데 커뮤니티(운영진이나 다른 참가자)가 어떤 도움을 주면 좋을까요? | `do_differently` 바로 뒤에 둔다 |
| `rejoin` | choice | 다음 기수에 다시 참여할 생각이 있나요? | 있다 / 조건이 맞으면 있다 / 없다 |
...
`{activity}`, `{artifact}` 는 설문의 `vars` 로 채운다.

| 스터디 | `activity` | `artifact` |
|---|---|---|
| 블로그 | 매주 글을 쓰는 데 | 글 |
| 리트코드 | 매주 문제를 푸는 데 | 풀이 |
...
공통 문항의 **문구나 보기를 고치는 것은 이전 기수와의 비교를 끊는 일**이다. 꼭 고쳐야 하면 새 `key` 로 추가하고 옛 것은 남긴다.
...
```sql
-- 스터디·기수별 목표 달성도 평균
SELECT s.name AS study, c.name AS cohort, AVG(CAST(a.value AS REAL)) AS avg
FROM answers a
